...
let source=match[1];
if(stage==='fragment'&&code.includes('TorusLight.fragment')){const ctx={};vm.runInNewContext(fs.readFileSync(path.join(root,'lightfield.js'),'utf8')+';this.Light=TorusLight',ctx);source=ctx.Light.fragment(source,['kinetic.js','sculptures.js','chiaroscuro.js','mechanisms.js','quasicrystal.js'].includes(file));}
