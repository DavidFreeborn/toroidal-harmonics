...
visit(root);fs.writeFileSync(path.join(root,'release.json'),JSON.stringify({revision:25,files},null,2)+'\n');
console.log('Release 25: '+files.length+' files, '+(files.reduce((sum,file)=>sum+file.bytes,0)/1048576).toFixed(2)+' MiB.');
